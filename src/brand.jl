...
# Forward base methods to Brand implementation
# take_sample!(svd_obj::IncrementalSVDBrand, u::Vector{Float64}; kwargs...) = take_sample!(svd_obj.base, u; kwargs...)

function take_sample!(svd_obj::IncrementalSVDBrand, u::Vector{Float64}; add_without_increase::Bool=false)
...
"""
    build_initial_svd!(svd_obj::IncrementalSVDBrand, u::Vector{Float64})

Build initial SVD for Brand algorithm.
"""
function build_initial_svd!(svd_obj::IncrementalSVDBrand, u::Vector{Float64})
    norm_u = norm(u)
...
"""
    build_incremental_svd!(svd_obj::IncrementalSVDBrand, u::Vector{Float64}, add_without_increase::Bool)

Brand-specific incremental SVD implementation.
"""
function build_incremental_svd!(svd_obj::IncrementalSVDBrand, u::Vector{Float64}, add_without_increase::Bool)
    # Compute projection error using Gram-Schmidt with re-orthogonalization
    e_proj = copy(u)
    
    # First Gram-Schmidt step
    tmp = svd_obj.base.U' * e_proj
    e_proj -= svd_obj.base.U * tmp
...
"""
    add_new_sample!(svd_obj::IncrementalSVDBrand, j::Vector{Float64}, A::Matrix{Float64}, W::Matrix{Float64}, sigma::Matrix{Float64})

Add new linearly independent sample for Brand algorithm.
"""
function add_new_sample!(svd_obj::IncrementalSVDBrand, j::Vector{Float64}, A::Matrix{Float64}, W::Matrix{Float64}, sigma::Matrix{Float64})
    n = svd_obj.base.num_samples
    ...
    if abs(check_orthogonality(svd_obj.Up)) > tol
        F = qr(svd_obj.Up)
...
Update spatial basis with singular value filtering.
"""
...
Update temporal basis with singular value filtering.
"""
function update_temporal_basis!(svd_obj::IncrementalSVDBrand)
    ...
    # Reorthogonalize if necessary
    if abs(check_orthogonality(svd_obj.base.basis_right)) > eps(Float64) * svd_obj.base.num_samples
